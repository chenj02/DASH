...
from scene.cameras import Camera, Sports_Camera, Camera_tech
import numpy as np
...
def loadSportsCam(args, id, cam_info, resolution_scale):
    orig_w, orig_h = cam_info.image.size
    ...
    return Sports_Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
...
def loadTechCam(args, id, cam_info, resolution_scale):
    orig_w, orig_h = cam_info.image.size
    ...
    return Camera_tech(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY,cxr=cam_info.cxr, cyr=cam_info.cyr,
                  image=gt_image, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id,
                  data_device=args.data_device if not args.load2gpu_on_the_fly else 'cpu', fid=cam_info.fid,
                  depth=cam_info.depth)

def cameraList_from_camInfos(cam_infos, resolution_scale, datatype, args):
    camera_list = []
    if datatype == "Technicolor":
        for id, c in enumerate(cam_infos):
            camera_list.append(loadTechCam(args, id, c, resolution_scale))
    elif datatype != "PanopticSports":
        for id, c in enumerate(cam_infos):
            camera_list.append(loadCam(args, id, c, resolution_scale))
    else:
        for id, c in enumerate(cam_infos):
            camera_list.append(loadSportsCam(args, id, c, resolution_scale))
    return camera_list
...
def camera_sports_to_JSON(id, camera: Sports_Camera):
    Rt = np.zeros((4, 4))
    Rt[:3, :3] = camera.R.transpose()
    Rt[:3, 3] = camera.T
    Rt[3, 3] = 1.0
