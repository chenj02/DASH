...
class GaussianModel:
    ...
    def save_dynamic_ply(self, path):
        ...
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    def save_static_ply(self, path):
        mkdir_p(os.path.dirname(path))
        dynamic = self._dynamic.detach().cpu().numpy()
        mask = dynamic.squeeze(1) == 0
        dynamic = dynamic[mask]
        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        xyz = xyz[mask]
    ...
    def load_ply(self, path, og_number_points=-1):
        ...
        self._dynamic = torch.tensor(dynamic.astype(bool), device="cuda") 
    ...
    def prune_points(self, mask):
        ...
        self._dynamic = self._dynamic[valid_points_mask] 
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
        ...
        self._scaling = optimizable_tensors["scaling"]
        self._rotation = optimizable_tensors["rotation"]
        self._dynamic = torch.cat((self._dynamic, new_dynamic), dim=0).to("cuda") 
    ...
    def dynamic_densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        n_init_points = self.get_xyz.shape[0]
        mask = self.get_dynamic
        mask = mask.squeeze(1)
        # Extract points that satisfy the gradient condition
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad_1 = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
        padded_grad_1[mask] = padded_grad[mask]
        selected_pts_mask = torch.where(padded_grad_1 >= grad_threshold, True, False)
        selected_pts_mask = torch.logical_and(selected_pts_mask,
                                              torch.max(self.get_scaling,
                                                        dim=1).values > self.percent_dense * scene_extent)
    ...
    def dynamic_densify_and_clone(self, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        mask = self.get_dynamic
        mask = mask.squeeze(1)
        mask = mask.bool()
    ...
    def dynamic_densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, disable_ws_prune=False):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.dynamic_densify_and_clone(grads, max_grad, extent)
        self.dynamic_densify_and_split(grads, max_grad, extent)
