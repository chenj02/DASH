...
class CameraInfoTech(NamedTuple):
    uid: int
    R: np.array
    T: np.array
    FovY: np.array
...
class SportsCameraInfo(NamedTuple):
    uid: int
    R: np.array
    T: np.array
    fx: float
...
def readColmapCamerasTechnicolor(cam_extrinsics, cam_intrinsics, images_folder, startime=0, duration=None):
    ...
    for idx, key in enumerate(cam_extrinsics): 
        ...
        for j in range(startime, startime+ int(duration)):
            ...
            image = Image.open(image_path)
            #image = PILtoTorch(image,None) # add

            cam_info = CameraInfoTech(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=width, height=height, fid=(j-startime)/duration, cxr=cxr, cyr=cyr)
            cam_infos.append(cam_info)
    sys.stdout.write('\n')
    return cam_infos
...
def readColmapSceneInfoTechnicolor(path, duration=50):
    ...
    cam_infos_unsorted = readColmapCamerasTechnicolor(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=path, duration=duration)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
