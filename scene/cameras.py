...
import numpy as np
from utils.graphics_utils import getWorld2View2, getProjectionMatrix, focal2fov, getProjectionMatrix_sports, getProjectionMatrixCV
...
class Sports_Camera(nn.Module):
    def __init__(self, colmap_id, R, T, fx, fy, cx, cy, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None):
        super(Sports_Camera, self).__init__()
        ...
        self.world_view_transform = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1).to(
            self.data_device)
        self.projection_matrix = getProjectionMatrix_sports(znear=self.znear, zfar=self.zfar, fx=self.fx, fy=self.fy,
                                                            cx=self.cx, cy= self.cy, w=self.image_width, h=self.image_height).transpose(0, 1).to(self.data_device)
        self.full_proj_transform = (
            self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        self.camera_center = self.world_view_transform.inverse()[3, :3]
...
class Camera_tech(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, cxr, cyr, image, gt_alpha_mask, image_name, uid,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None):
        super(Camera_tech, self).__init__()
        ...
        if cyr != 0.0 :
            self.cxr = cxr
            self.cyr = cyr
            self.projection_matrix = getProjectionMatrixCV(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy, cx=cxr, cy=cyr).transpose(0,1).to(self.data_device)
        else:
            self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx,
                                                     fovY=self.FoVy).transpose(0, 1).to(self.data_device)
        self.full_proj_transform = (
